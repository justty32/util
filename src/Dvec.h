...
#define SERIALIZE_FUNCS_OUT_OF_CLASS(ClassName)\
...
// put it inside, file with class members, ex. class ee{ SERIAL..(x,y,c) }
#define SERIALIZE_FUNCS_IN_CLASS_PUBLIC(...)\
uio::serialize::Dvec to_dvec(){return uio::serialize::to_dvec_pack(__VA_ARGS__);}\
void from_dvec(uio::serialize::Dvec dvec){uio::serialize::from_dvec_pack(dvec, __VA_ARGS__);}
...
namespace uio{
	namespace serialize{
		...
		Dvec to_dvec(T& target) {
			...
			{
				return to_dvec_orderer_container(target);
			}
			else if constexpr (is_specialization <T, std::pair>::value) {
				return to_dvec_pair(target);
			}
			else if constexpr (is_specialization <T, std::tuple>::value) {
				return to_dvec_tuple(target);
			}
			else if constexpr (std::is_same<T, std::string>::value) {
				return to_dvec_str(target);
			}
			else {
		...
		T from_dvec(Dvec dvec) { 
			if constexpr (is_specialization <T, std::vector>::value
				...
				|| std::is_array<T>::value)
			{
				return from_dvec_orderer_container<T>(dvec);
			}
			else if constexpr (is_specialization <T, std::pair>::value) {
				return from_dvec_pair<T>(dvec);
			}
			else if constexpr (is_specialization <T, std::tuple>::value) {
				return from_dvec_tuple<T>(dvec);
			}
			else if constexpr (std::is_same <T, std::string>::value) {
				return from_dvec_str(dvec);
			}
			else {
				static_assert(std::is_pod<T>::value, "T is not POD type, please define its to_dvec");
